...
#'  getLivrosPublicados(xmlsLattes[[2]])
#'
...
getLivrosPublicados <- function(curriculo) {
    ...
    xml_find_all(curriculo, ".//LIVRO-PUBLICADO-OU-ORGANIZADO") %>>%
        map(~ xml_find_all(., ".//DADOS-BASICOS-DO-LIVRO")) %>>%
        map(~ xml_attrs(.)) %>>%
        map(~ bind_rows(.)) %>>%
    ...
    xml_find_all(curriculo, ".//LIVRO-PUBLICADO-OU-ORGANIZADO") %>>%
        map(~ xml_find_all(., ".//DETALHAMENTO-DO-LIVRO")) %>>%
        map(~ xml_attrs(.)) %>>%
        map(~ bind_rows(.)) %>>%
        map(~ janitor::clean_names(.)) %>>%
        (. -> detalhamento)
    ...
    xml_find_all(curriculo, ".//LIVRO-PUBLICADO-OU-ORGANIZADO") %>>%
        ...
        map(~ if (nrow(.x) == 0) { tibble(palavras_chave = NA) } else {.x})  %>>%
        (. -> palavras_chave)
