...
As example, 500 random curricula data `lattesXML` imported as an R list.
...
lapply(lattesXML, getArtigosPublicados) %>>% 
    ...
    normalizeYears %>>% 
    select(titulo.do.artigo,ano.do.artigo,issn,titulo.do.periodico.ou.revista,id) %>>% 
    left_join( lapply(lattesXML, getDadosGerais) %>>% bind_rows %>>% select(id,nome.completo,pais.de.nascimento)) %>>%   
    slice(1:10)
```
