...
```{r eval=F, include=T}
curriculo <- xml2::read_xml('4984859173592703.zip')
```
...
files <- list.files(path = '../data/', pattern = '*.xml$|*.zip', full.names = T)

files <- fs::dir_ls(path = '../data/', regexp = '*.xml|*.zip')
```
...
Adding an extra layer of complexity, I will use pipe `%>>%` operator from `pipeR` package. Programming using the pipe operator `%>>%` allows faster coding and clearer syntax.
Where `. -> curriculos` means the result was saved to `curriculos` object.
...
artigos_publicados2 %>>%
    left_join(livros_publicados2) %>>%
    left_join(capitulos_livros2)
```
...
artigos_publicados2 %>>%
    left_join(livros_publicados2) %>>%
    left_join(capitulos_livros2) %>>%
    left_join(dados_gerais %>>% select(id, nome_completo)) %>>%
    relocate(nome_completo) 
```
